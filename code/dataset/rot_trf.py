...
def rect_from_lm(lm7, scale=1):
    lms = np.array(lm7).ravel()
    if len(lms) == 14:
        lms = lms.reshape(7, -1)
    ...
    face_rect = [max(0, face_center[0] - fsize // 2), max(0, face_center[1] - fsize // 2), fsize, fsize]
...
def lm_angle(lms):
    if lms.shape[0] == 7:
        ...
        raise NotImplementedError(f"incorrect landmark shape: supported [5, 2] or [7, 2], got: {lms.shape}")
...
def _get_size_value(size_param):
    ...
    elif (isinstance(size_param, tuple) or isinstance(size_param, list)) and len(size_param) == 2:
        assert isinstance(
            size_param[0], int), f"Incorrect size[0] type, int expected, got {type(size_param[0])}"
        assert isinstance(
            size_param[1], int), f"Incorrect size[1] type, int expected, got {type(size_param[1])}"
        assert size_param[
            0] > 0, f"Incorrect input: size[0] should be > 0, got {size_param[0]}"
        assert size_param[
            1] > 0, f"Incorrect input: size[1] should be > 0, got {size_param[1]}"
        return size_param
    else:
...
class MetaRandomNoise:
    ...
    def __call__(self, sample: Dict) -> Dict:
        ...
        lm_noise_x = np.array(
            [np.float32((2 * random.random() - 1) * lm_shift_size[0]) for _ in lm_array])
        lm_array[:, 0] += lm_noise_x

        lm_noise_y = np.array(
            [np.float32((2 * random.random() - 1) * lm_shift_size[1]) for _ in lm_array])
...
class FaceRegionXT(ScalingTransform):
    ...
    """
    def __init__(self, size: Tuple[int, int] = None, scale_rect_hw: Tuple[int, int] = (1, 1), crop: Tuple[int, int] = None,
                 interpolation=cv2.INTER_LINEAR, p=0.0, scale_delta=0.0, square=False):
        ...
        assert scale_rect_hw[
            1] > 0, f"scale factor for X should be greater than 0, got {scale_rect_hw[1]:.2f}"
        assert scale_rect_hw[
            0] > 0, f"scale factor for Y should be greater than 0, got {scale_rect_hw[0]:.2f}"
    ...
    def get_affine_transform(self, meta: Dict) -> Tuple[np.ndarray, Tuple[int, int]]:
        ...
        reset_mat = np.array(
            [[1, 0, dsize[0] / 2], [0, 1, dsize[1] / 2], [0, 0, 1]], dtype=np.float32)
    ...
    def update_meta(self, meta: Dict, affine_mat: np.ndarray) -> Dict:
        ...
        rect_array[:2] = rect_array[None,
                                    :2] @ affine_mat[:2, :2].T + affine_mat[:2, -1:].T
    ...
    def __call__(self, sample: Dict) -> Dict:
        ...
        # crop image
        dst_image = cv2.warpAffine(sample["image"].copy(
        ), affine_mat, dst_size, flags=self.interpolation, borderMode=cv2.BORDER_REPLICATE)
